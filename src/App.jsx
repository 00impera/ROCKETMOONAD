      ...
      <div style={{ padding: "20px 20px 80px" }}>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: "8px", marginBottom: "14px" }}>
          ...
          <div style={{ flex: 1, minWidth: 180, display: "flex", alignItems: "center", gap: "10px", background: "var(--bg-panel)", border: "0.5px solid var(--green-dim)", borderRadius: "10px", padding: "10px 14px" }}>
            ...
            <a href={DEX_URL} target="_blank" rel="noreferrer" style={{ marginLeft: "auto", fontSize: "10px", color: "#00FF88", border: "0.5px solid #00FF8844", borderRadius: "6px", padding: "3px 8px", textDecoration: "none" }}>📊</a>
          ...
          <div style={{ flex: 1, minWidth: 180, display: "flex", alignItems: "center", gap: "10px", background: "var(--bg-panel)", border: "0.5px solid var(--green-dim)", borderRadius: "10px", padding: "10px 14px" }}>
            <div>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
                ...
                {change !== null && <span style={{ color: change > 0 ? "var(--green)" : "var(--red)", fontSize: "11px" }}>{change > 0 ? "▲" : "▼"} {Math.abs(change).toFixed(2)}%</span>}
              </div>
            </div>
            <span style={{ fontSize: "9px", color: "#333", marginLeft: "auto" }}>↻30s</span>
          </div>
        </div>
        ...
        {tab === "dex" && (
          <div>
            <div style={{ fontSize: "13px", fontWeight: "600", color: "var(--text-primary)", marginBottom: "12px" }}>
              📊 DEX Live · {DEX_PAIRS.length} Pairs · Monad On-Chain
            </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(300px,1fr))", gap: "8px" }}>
              {filteredDex.map(p => {
                ...
                const hasPrice = !!pd;
                return (
                  <div key={p.pair} className="dex-card" style={{ borderLeftColor: p.color }}
                    onClick={() => setSelectedPair(selectedPair === p.pair ? null : p.pair)}>
                    <div className={`dex-dot ${hasPrice ? "" : "offline"}`} />
                    {/* Token icon or colored circle */}
                    <div style={{ width: 36, height: 36, borderRadius: "50%", background: p.color + "22", border: `2px solid ${p.color}`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, fontSize: 11, fontWeight: 700, color: p.color, fontFamily: "monospace" }}>
                      {p.symbol.slice(0, 3)}
                    </div>
                    <div style={{ flex: 1 }}>
