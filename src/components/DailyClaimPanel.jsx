...
// ── Selectors ─────────────────────────────────────────────────────────────────
// We store claim state in localStorage per wallet to avoid any on-chain read
// that could trigger the OwnableUnauthorizedAccount revert.
// Key: "dailyClaim:<walletAddr>" → JSON array of 7 booleans, resets each Mon.

const DAYS = ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"];

/** Returns 0=Mon … 6=Sun (Monad week starts Monday) */
function todayMonIndex() {
  const d = new Date().getDay(); // 0=Sun,1=Mon…6=Sat
  return d === 0 ? 6 : d - 1;
}

/** ISO week string "YYYY-Www" for cache-busting on new week */
function isoWeek() {
  const d  = new Date();
...
function useDailyClaim(account, sendTx) {
  ...
  const claimDay = useCallback(async (idx) => {
    ...
    setMsg("");
    try {
      // claimDailyReward() — selector 0x (add your contract's selector here)
      // If your staking contract has no daily claim function, this calls claimRewards()
      // Replace method string with your actual daily-claim function signature.
